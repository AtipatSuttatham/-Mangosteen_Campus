...
- **Authentication**: JWT ผ่าน `djangorestframework-simplejwt` (frontend เรียก backend ผ่าน path `/api` ที่ proxy ไปหา Django จึงเป็น origin เดียวกัน ไม่ต้องตั้ง CORS — เลือก JWT เพื่อให้ frontend ตรวจสิทธิ์จาก token ได้ และแยก frontend/backend ชัดเจน) ใช้ access/refresh token, เก็บ role ไว้ใน user model แล้วใส่ role ลงใน token claims ตอนออก token (`backend/accounts/tokens.py`) เพื่อให้ frontend แสดงเมนูตามบทบาทได้ — สิทธิ์จริงของข้อมูล backend ตรวจจากฐานข้อมูลทุกครั้ง สัญญา API อยู่ที่ `docs/api-auth.md`
  ...
  - **การเก็บ token ฝั่ง frontend (ตัดสินใจแล้ว)**: access token เก็บ**ในหน่วยความจำ**เท่านั้น (ไม่ใช้ localStorage) ส่วน refresh token เก็บใน **httpOnly cookie** — เพื่อกัน XSS ขโมย token. หมุนเวียน refresh token ทุกครั้งที่แลก (เขียนเองใน `accounts/tokens.py` เพื่อให้ role ใหม่มีผลทันที) และใช้ตาราง blacklist ของ simplejwt เก็บเฉพาะใบที่ถูกเพิกถอนแล้ว — **ไม่เก็บ refresh token ที่ยังใช้งานได้ลงฐานข้อมูล** (ห้ามใช้ `RefreshToken.for_user` เพราะมันเก็บตัว token เต็ม ๆ). ตอน deploy ต้องให้ frontend/backend เป็น same-site (frontend proxy `/api` ไปหา backend หรือใช้โดเมนเดียวกัน) เพื่อไม่ให้เบราว์เซอร์บล็อก cookie — ผูกกับการเลือก hosting (ยังไม่ตัดสินใจ)
