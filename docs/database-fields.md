...
| field | เก็บอะไร |
|---|---|
| `email` | อีเมล (ไม่ซ้ำ) — ใช้ login ได้ — เก็บตัวพิมพ์เล็กเสมอ |
| `student_or_staff_id` | รหัสนักศึกษา/พนักงาน (ไม่ซ้ำ, ว่างได้) — ใช้ login ได้ — ตัดช่องว่างหัวท้าย แยกตัวพิมพ์ ห้ามมี `@` ไม่มีรหัส = ว่าง (NULL) |
...
|---|---|
| `user` | โทเคนนี้ของใคร |
| `token_hash` | ค่าแฮช SHA-256 ของโทเคนสุ่ม (ไม่ซ้ำ) — ตัวโทเคนจริงอยู่ในลิงก์อีเมลเท่านั้น ไม่เก็บในฐานข้อมูล |
...
| `invite_code` | รหัสเข้าเรียน (ไม่ซ้ำ, ว่างได้) — 8 ตัวอักษร (A–Z ตัด I L O และ 2–9) แสดงเป็น XXXX-XXXX กรอกไม่สนตัวพิมพ์ |
