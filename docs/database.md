...
| `used_at` | datetime, `null=True` | ใช้แล้วใช้ซ้ำไม่ได้ (ตอนออกโทเคนใหม่ให้บัญชี+จุดประสงค์เดียวกัน โทเคนเก่าที่ยังไม่ใช้จะถูกตั้ง `used_at` เพื่อยกเลิก) |
| `created_at` | datetime, auto | เวลาออกโทเคน — **เพิ่มตอนทำก้อน c1** เพื่อบังคับกฎ "ขอลิงก์ใหม่ได้ทุก 60 วินาที" (สเปก v1.4 เดิมไม่มี) |
